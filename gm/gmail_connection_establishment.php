<?php
class gmail_connection_establishment 
{
    public $oauth_consumer_key;
    public $oauth_consumer_secret;
    public $progname;
    public $debug;
    public $your_callback_url;

    function __construct($your_consumer_key, $your_consumer_secret, $argarray, $debug, $your_callback_url) {
        $this->oauth_consumer_key = $your_consumer_key;
        $this->oauth_consumer_secret = $your_consumer_secret;
        $this->progname = $argarray;
        $this->debug = $debug; // Set to 1 for verbose debugging output
        $this->callback = $your_callback_url;
    }
    ...
    function oauth_http_build_query($params, $excludeOauthParams=false) {
        ...
        if (!empty($params)) {
            ...
            $keys = $this->vpb_encoder(array_keys($params));
            $values = $this->vpb_encoder(array_values($params));
    ...
    function build_oauth_header($params, $realm='') {
        ...
        foreach ($params as $k => $v) {
            if (substr($k, 0, 5) == 'oauth') {
                $header .= ',' . $this->vpb_encoder($k) . '="' . $this->vpb_encoder($v) . '"';
            }
        }
        return $header;
    }
    function oauth_compute_plaintext_sig($your_consumer_secret, $token_secret) {
        return ($your_consumer_secret . '&' . $token_secret);
    }
    function oauth_compute_hmac_sig($http_method, $url, $params, $your_consumer_secret, $token_secret) {

        $base_string = $this->signature_base_string($http_method, $url, $params);
        $signature_key = $this->vpb_encoder($your_consumer_secret) . '&' . $this->vpb_encoder($token_secret);
    ...
    function signature_base_string($http_method, $url, $params) {
        ...
        $base_string = $this->vpb_encoder(strtoupper($http_method)) . '&' .
                $this->vpb_encoder($this->normalize_url($url)) . '&' .
                $this->vpb_encoder($this->oauth_http_build_query($params));
     ...
     */
    function vpb_encoder($raw_input)
	{
        if (is_array($raw_input)) 
		{
            //return array_map($this->vpb_encoder, $raw_input);
            return array_map(array($this, 'vpb_encoder'), $raw_input);
            // return $this->vpb_encoder($raw_input);
        } 
    ...
    function vpb_decoder($raw_input) 
	{
...
class import_gmail_contacts 
{
    ...
    function get_access_token($oauth, $request_token, $request_token_secret, $oauth_verifier, $usePost=false, $useHmacSha1Sig=true, $passOAuthInHeader=true)
	{
		...
		{
            $params['oauth_signature_method'] = 'HMAC-SHA1';
            $params['oauth_signature'] = $oauth->oauth_compute_hmac_sig($usePost ? 'POST' : 'GET', $url, $params,$oauth->oauth_consumer_secret, $request_token_secret);
        } 
		else 
		{
            echo "signature mathod not support";
    ...
    function importGmailContacts($oauth, $access_token, $access_token_secret, $usePost=false, $passOAuthInHeader=true,$max_email_number) 
	{
        $retarr = array();  // return value
        $response = array();
        ...
        $params['alt'] = 'json';
        $params['max-results'] = $max_email_number;
        $params['oauth_version'] = '1.0';
        $params['oauth_nonce'] = mt_rand();
        $params['oauth_timestamp'] = time();
        $params['oauth_consumer_key'] = $oauth->oauth_consumer_key;
